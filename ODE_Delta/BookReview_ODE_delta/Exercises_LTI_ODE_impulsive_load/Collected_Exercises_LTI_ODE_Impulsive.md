...
Example 8.22. Find the inverse Laplace transform of
$$
X(s) = \frac{2s+1}{s+3}
$$
for (a) ROC: $\text{Re}\{s\} < -3$; (b) ROC: $\text{Re}\{s\} > -3$
$$
\text{(a)}\quad x(t) = 2\delta(t) + 5e^{-3t}u(-t)
$$
$$
...
##### Krishnaveni & Rajeswari — Signals and systems, p.473

Example 8.24. Find the inverse Laplace transform of
$$
...
Example 8.21. Find the inverse Laplace transform of
$$
...
Example 8.23. Find the inverse Laplace transform of
$$
X(s) = \frac{1}{(s+5)(s-3)}
$$
for (a) ROC: $-5 < \text{Re}\{s\} < 3$; (b) ROC: $\text{Re}\{s\} > 3$; (c) ROC: $\text{Re}\{s\} < -5$
$$
\text{(a)}\quad x(t) = -\frac{1}{8}e^{-5t}u(t) - \frac{1}{8}e^{3t}u(-t)
$$
$$
\text{(b)}\quad x(t) = \frac{1}{8}\left(e^{3t} - e^{-5t}\right)u(t)
$$
$$
...
##### Krishnaveni & Rajeswari — Signals and systems, p.482

Example 8.31. Using Laplace transform, solve the differential equation with zero initial conditions
$$
...
##### Krishnaveni & Rajeswari — Signals and systems, p.483

Example 8.32. Find the output $y(t)$ of an LTI system represented. Assume that the system is initially at rest.
$$
...
##### Kamaraju & Narasimham — Linear systems: Analysis and applications, p.105

**Example 4.24.** Find the inverse transform of
$$
...
##### Krishnaveni & Rajeswari — Signals and systems, p.476
...
$$
G(s) = \frac{s+1}{s(s^2+\omega^2)} = \frac{1}{\omega^2 s} + \frac{1}{s^2+\omega^2} - \frac{s}{\omega^2 (s^2+\omega^2)}
$$
$$
...
$$
Y(s) = \frac{KA}{s(s+a)(s+b)} \implies y(t) = (K_1 + K_2 e^{-at} + K_3 e^{-bt})\,1(t)
$$
$$
...
##### Klee & Allen — Simulation of dynamic systems with MATLAB® and Simulink®, p.175, Formula (4.168)

$$
H(s) = \frac{s^2 + 3s + 1}{s(s+1)(s+3)(s+5)} \implies \frac{1}{15} + \frac{1}{8} e^{-t} + \frac{1}{12} e^{-3t} - \frac{11}{40} e^{-5t}, \quad t \geq 0
$$
*[Editorial note: as transcribed, the source's denominator is printed as $(s+1)(s+3)(s+5)$, but partial-fraction expansion of $H(s)=(s^2+3s+1)/((s+1)(s+3)(s+5))$ gives $-\frac{1}{8(s+1)} - \frac{1}{4(s+3)} + \frac{11}{8(s+5)}$, which does not match the stated closed-form answer at all. Restoring a missing factor of $s$ in the denominator, $H(s)=(s^2+3s+1)/(s(s+1)(s+3)(s+5))$, gives partial fractions $\frac{1}{15s}+\frac{1}{8(s+1)}+\frac{1}{12(s+3)}-\frac{11}{40(s+5)}$ — matching the stated answer's coefficients exactly. This is treated as a transcription slip (a dropped $s$ factor) rather than an error in the answer itself; note also that with this correction the system is fourth-order (not second-order as the source's original placement might suggest), which is why this entry is classified here under Higher-order equations.]*

*[WolframAlpha: `InverseLaplaceTransform[(s^2 + 3 s + 1)/(s (s + 1) (s + 3) (s + 5)), s, t]` — Returns $h(t) = \frac{1}{15} + \frac{1}{8} e^{-t} + \frac{1}{12} e^{-3t} - \frac{11}{40} e^{-5t}$, matching the book*]

*[Jump condition at $t=0$: $H(s)=(s^2+3s+1)/(s(s+1)(s+3)(s+5))$ has denominator $s(s+1)(s+3)(s+5)=s^4+9s^3+23s^2+15s$ and numerator $s^2+3s+1$ (relative degree $2$), corresponding to the fourth-order ODE $y''''+9y'''+23y''+15y'=\delta''(t)+3\delta'(t)+\delta(t)$. Phase vector $\mathbf y=(y,y',y'',y''')$ changes by $\Delta\mathbf y(0)=(0,1,-6,32)$.]*
...
**Klee** p.169 Formula (4.125) consider the firstorder system differential equation model where the input u - unit impulse and the system is initially at rest
$$
\frac{dy}{dt} + 2y = \frac{du}{dt} + u
$$

**Klee** p.172 Formula (4.142)
$$
\ddot{x} + 2\dot{x} + 4x = \delta(t)
$$

**Klee** p.187 EXERCISES 4.7(b)
...
**Dobrushkin** p.316 Example 5.6.7. "A mass of 0.2 kg is attached to a spring with stiffness coefficient 104 newtons per meter. Suppose that at t = 0 the mass, which is motionless and in equilibrium position, is struck by a hammer of mass 0.25 kg, traveling at 40 m per sec. Find the displacement if the damping coefficient is approximately 0.02 kg/sec."

**Dobrushkin** p.317 Problems.5 Solve the IVPs involving the Dirac delta-function. The initial conditions are assumed to be homogeneous
...
Dobrushkin, V. A. (2022). Applied differential equations: The primary course (2nd ed.). CRC Press. https://doi.org/10.1201/9781003299592
...
Klee, H., & Allen, R. (2011). Simulation of dynamic systems with MATLAB® and Simulink® (2nd ed.). CRC Press, Taylor & Francis Group. (ISBN: 978-1-4398-3673-6)
