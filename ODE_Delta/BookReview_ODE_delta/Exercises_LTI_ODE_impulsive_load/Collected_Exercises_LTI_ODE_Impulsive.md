...
\end{cases}
$$
*[WolframAlpha verification (Wolfram Language): paste either line below directly into the input box at [wolframalpha.com](https://www.wolframalpha.com) — it auto-detects Wolfram Language syntax.*
```
2 y''[t] + y'[t] + 2 y[t] == Delta[t - 5], y[0] == 0, y'[0] == 0
```
*Return $y(t) = \frac{2 \, e^{5/4 - t/4} \, u(t-5) \, \sin\left(\frac{1}{4} \, \sqrt{15} \, (t-5)\right)}{\sqrt{15}}$*
...
e^{-t} + e^{-(t-1)}, & t \ge 1
\end{cases}$$
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{y'[t] + y[t] == DiracDelta[t - 1], y[0] == 1}, y[t], t]
```
*returns* `UnitStep[t - 1]*Exp[1 - t] + Exp[-t]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{x''[t] + 4 x[t] == 8 DiracDelta[t - 2 Pi], x[0] == 3, x'[0] == 0}, x[t], t]
```
*returns* `4*UnitStep[t - 2*Pi]*Sin[2*t] + 3*Cos[2*t]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
Limit[D[x[t] /. First[DSolve[{x''[t] + x'[t] + 2 x[t] == DiracDelta[t], x[0] == 0, x'[0] == 0}, x[t], t]], t], t -> 0, Direction -> "FromAbove"]
```
*returns* `1`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{tau x'[t] + x[t] == A DiracDelta[t], x[0] == x0}, x[t], t]
```
*returns* `A*Exp[-t/tau]/tau + x0*Exp[-t/tau]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
Limit[y[t] /. First[DSolve[{y'[t] + k y[t] == DiracDelta[t], y[0] == 0}, y[t], t]], t -> 0, Direction -> "FromAbove"]
```
*returns* `1`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
InverseLaplaceTransform[(2 s + 1)/((s + 1)^2 + 2^2), s, t]
```
*returns* `-1/2*Exp[-t]*Sin[2*t] + 2*Exp[-t]*Cos[2*t]`, *matching the closed form above.]*
...
```
*returns* `DiracDelta[t] + Exp[-2*t] + 2*Exp[-3*t]`, *matching the closed form above.]*

p.240 Example 3.9 has been omitted from this compilation. As transcribed, its differential equation and impulse response are byte-for-byte identical to Example 3.12 below (p.254), which strongly suggests a transcription duplication rather than two distinct worked examples. Since the true text of Example 3.9 could not be independently re-verified against the source, it is left out here rather than guessed at.
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
InverseLaplaceTransform[1/(s^2 + 5 s + 6), s, t]
```
*returns* `Exp[-2*t] - Exp[-3*t]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
InverseLaplaceTransform[1/(s^3 + 6 s^2 + 11 s + 6), s, t]
```
*returns* `(1/2)*Exp[-t] - Exp[-2*t] + (1/2)*Exp[-3*t]`, *matching the closed form above.]*
...
```
*returns* `3*t*Exp[-t] + DiracDelta[t] - 3*Exp[-t]`, *matching the closed form above.]*
...
```
*returns* `DiracDelta[t] + 2*Exp[-t] + Exp[-2*t]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
InverseLaplaceTransform[1/(s^2 + 4 s + 10), s, t]
```
*returns* `(1/6)*6^(1/2)*Exp[-2*t]*Sin[6^(1/2)*t]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{y''[t] + 5 y'[t] + 6 y[t] == DiracDelta[t - Pi] - DiracDelta[t - 2 Pi], y[0] == 0, y'[0] == 0}, y[t], t]
```
*returns* `(-Exp[-3t+3Pi] + Exp[-2t+2Pi])*UnitStep[t-Pi] - (-Exp[-3t+6Pi] + Exp[-2t+4Pi])*UnitStep[t-2Pi]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{y''''[t] + 2 y'''[t] - y''[t] - 2 y'[t] == DiracDelta[t], y[0] == 1, y'[0] == 0, y''[0] == 0, y'''[0] == 0}, y[t], t]
```
*returns* `1/2 + (1/2)*Exp[-t] + (1/6)*Exp[t] - (1/6)*Exp[-2*t]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{y''[t] + 5 y'[t] + 6 y[t] == 3 DiracDelta[t - 2] - 4 DiracDelta[t - 4], y[0] == 0, y'[0] == 0}, y[t], t]
```
*returns* `(3*Exp[4-2t] - 3*Exp[6-3t])*UnitStep[t-2] - (4*Exp[8-2t] - 4*Exp[12-3t])*UnitStep[t-4]`, *matching the closed form above.]*
...
```
*returns* `(-Exp[-2*t] + 2*Exp[-3*t])*UnitStep[t]`, *matching the closed form above.]*
...
```
*returns* `(-Exp[-t] + 2*Exp[-2*t])*UnitStep[t]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
InverseLaplaceTransform[(3 s + 5)/(s + 2), s, t]  (* (a) *)
InverseLaplaceTransform[(s + 4)/(s (s + 2)), s, t]  (* (b) *)
InverseLaplaceTransform[s/(s + 1)^2, s, t]  (* (c) *)
```
*returns* `-UnitStep[t]*Exp[-2t]+3*DiracDelta[t]  ;  (2-Exp[-2t])*UnitStep[t]  ;  (1-t)*UnitStep[t]*Exp[-t]  (respectively)`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{y''[t] + 4 y'[t] + 3 y[t] == 0, y[0] == 1, y'[0] == 2}, y[t], t]
```
*returns* `(1/3 + 3*Exp[-t] - 7/3*Exp[-3*t])*UnitStep[t]  (checked as the zero-input/IC-driven part only -- x(t) not specified in the source excerpt, see note below)`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{y''[t] + 2 y'[t] + 5 y[t] == DiracDelta[t], y[0] == 0, y'[0] == 0}, y[t], t]
```
*returns* `(1/2)*Exp[-t]*Sin[2*t]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{x''[t] + 9 x[t] == 3 DiracDelta[t - Pi], x[0] == 1, x'[0] == 0}, x[t], t]
```
*returns* `for t>Pi: -Sin[3*t] + Cos[3*t]  (matches cos3t - sin3t, since sin(3(t-Pi)) = -sin(3t))`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{y''[t] + w0^2 y[t] == f0 DiracDelta[t - t0], y[0] == y0, y'[0] == 0}, y[t], t]
```
*returns* `y0*Cos[t*w0] + (f0/w0)*UnitStep[t-t0]*Sin[w0*(t-t0)]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{y''[t] + 2 y'[t] + 2 y[t] == DiracDelta[t], y[0] == 0, y'[0] == 0}, y[t], t]  (* c = 0 case *)
```
*returns* `Exp[-t]*Sin[t]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{y''[t] - y[t] == -20 DiracDelta[t - 3], y[0] == 1, y'[0] == 0}, y[t], t]
```
*returns* `-20*UnitStep[t-3]*Sinh[t-3] + Cosh[t]`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
DSolve[{y''[t] + 4 y[t] == DiracDelta[t - Pi] - DiracDelta[t - 2 Pi], y[0] == 0, y'[0] == 0}, y[t], t]
```
*returns* `(-1/2*UnitStep[t-2Pi] + (1/2)*UnitStep[t-Pi])*Sin[2*t]`, *matching the closed form above.]*
...
```
*returns* `Exp[-t/T]/T`, *matching the closed form above.]*
...
Assuming[0 < zeta < 1 && wn > 0,
  DSolve[{y''[t] + 2 zeta wn y'[t] + wn^2 y[t] == wn^2 DiracDelta[t], y[0] == 0, y'[0] == 0}, y[t], t]]
(* repeat with zeta == 1, then with zeta > 1, to get the other two branches *)
```
*returns* `matches the wn/Sqrt[1-zeta^2] Exp[-zeta wn t] Sin[...] form (0<=zeta<1), wn^2 t Exp[-wn t] (zeta=1), and the two-exponential form (zeta>1) shown above, respectively`, *matching the closed form above.]*
...
*[WolframAlpha verification: paste into the input box at wolframalpha.com —*
```
m = 10; k = 9000; c = 18; F0 = 10000; dt = 0.005;
wn = Sqrt[k/m]; xi = c/(2 Sqrt[m k]); wd = wn Sqrt[1 - xi^2];
N[{wn, xi, wd, xi wn, (F0 dt)/(m wd)}]
```
*returns* `{30., 0.03, 29.9865, 0.9, 0.166742}  -- matches the book's 29.986, 0.9, 0.1667`, *matching the closed form above.]*
...
```
*returns* `(1/12)*t^3*(t + 2)*Exp[2*t]`, *matching the closed form above.]*
...
```
*returns* `(1/6)*t^3 + (1/6)*(t - 3)^3*UnitStep[t - 3]`, *matching the closed form above.]*
...
```
*returns* `(1/4)*t*Sin[2*t]`, *matching the closed form above.]*
*[Editorial note: as transcribed, the source used the variable "l" in place of "t" ($\frac{1}{4} l \sin 2l$), which is not a function of $t$ and does not correspond to any standard Laplace pair. Read as an OCR/transcription misread of "t", $\frac{1}{4}t\sin2t$ is exactly the standard inverse Laplace transform of $s/(s^2+4)^2$ (confirmed symbolically), and is used here as a high-confidence correction.]*
...
```
*returns* `Exp[t] - Exp[-t]*Sin[2*t] - Exp[-t]*Cos[2*t]`, *matching the closed form above.]*
...
```
*returns* `(1/8)*Sin[t] - 1/24*Sin[3*t] + (1/8)*Cos[t] - 1/8*Cos[3*t]`, *matching the closed form above.]*
